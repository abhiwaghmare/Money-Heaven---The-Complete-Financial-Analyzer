...
$(function(){
	...
	$('#search #typeahead').typeahead({
		...
		},
		{
			...
			templates: {
			    header: '<p class="league-name">USD Cryptocurrencies</p>',
		...
		},
{
  ...
  templates: {
    header: '<p class="league-name" style="color:#97c950">USD Cryptocurrencies</p>',
    suggestion: Handlebars.compile('<p class="sugg-title"><strong>{{name}} - {{slug}}</strong></p>'),
  }
});
	...
	$('.typeahead').on('typeahead:selected', function (e, datum) {
		console.log(datum);
		window.location.href = '/searchCrypt/' + datum.se + '/' + datum.slug + '/';
	});
